...
def plot_model_latent_space(model, X_train, y_train):
  ...
  with open('tsne_emb_fl_autoencoder.pickle', 'wb') as handle:
...
def main():
  ...
  local_models = [generate_autoencoder_model(dataX[0].shape) for i in range(n_clients)]
  global_model = generate_autoencoder_model(dataX[0].shape)
  total_rounds = 5
  ...
  for round_id in range(total_rounds):
    ...
    for i, (train_index, test_index) in enumerate(splits):
      ...
      model.fit(X_train, X_train, epochs=100, shuffle=False, batch_size=16)
